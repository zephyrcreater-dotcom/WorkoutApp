...
function App() {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden pb-24 text-white lg:pb-0">
  ...
  function startOffProgramSession() {
    ...
    const session: WorkoutSession = {
      ...
      loggedExercises: offProgramBuilder.exercises.map((item, index) => ({
        ...
        weakPointTags: [],
        offProgram: true,
        offProgramPlannedSets: Array.from({ length: item.targetSets || 3 }, (_, i) => ({
          id: createId("pset"),
          ...
          {offProgramBuilder.exercises.length > 0 ? (
            <div className="mb-4 space-y-3">
              {offProgramBuilder.exercises.map((item, idx) => {
                ...
                const lastLog = (db.exercisePerformanceLogs ?? []).filter((l) => l.exerciseId === item.exerciseId && l.userId === user.id).at(-1);
                return (
              ...
              <ExercisePicker
                ...
                onPick={(exercise) => {
                  ...
                  setOffProgramBuilder((b) => ({
                    ...b,
                    exercises: [...b.exercises, { exerciseId: exercise.id, targetSets: 3, targetReps: 8, targetRpe: 7 }],
                  }));
                }}
              />
            </div>
...
function LiveLogger({
  ...
  const nextPlannedSet = planned?.plannedSets[effectiveSetIndex];
  ...
  }, [adjustedWeight, draftKey, lastSet, nextPlannedSet, selectedSetIndex]);
  ...
  const isEditingPastSet = selectedSetIndex !== null && selectedSetIndex < currentSetIndex;
  // Never run recommendation on skipped sets — they're not real performance data
  const lastNonSkippedSet = liveExerciseLog.sets.filter((s) => !s.skipped).at(-1);
  const nextSetResult = lastNonSkippedSet
    ? algNextSetAdjustment({ user, exercise: liveExercise, loggedSet: lastNonSkippedSet, nextPlannedSet, setsCompletedThisExercise: liveExerciseLog.sets.filter((s) => !s.skipped).length })
    : undefined;
  ...
  const primaryActionLabel = isEditingPastSet ? "Update Set" : primaryAction === "finish-workout" ? "Finish Workout" : primaryAction === "finish-exercise" ? "Finish Exercise" : "Next Set";
  ...
  function logSet(rating: SetRating = setDraft.setRating, afterAction: "stay" | "next-exercise" | "finish-workout" = "stay") {
    ...
    if (isWeightBased && actualWeight === 0 && !isEditingPastSet) {
      skipSet();
      return;
    }

    if (isEditingPastSet && selectedSetIndex !== null) {
      // Edit-in-place: update the existing set at selectedSetIndex, don't advance cursor
      void updateDb((draft) => {
        const target = draft.sessions.find((item) => item.id === liveSession.id);
        const log = target?.loggedExercises.find((item) => item.id === liveExerciseLog.id);
        if (log?.sets[selectedSetIndex]) {
          const existing = log.sets[selectedSetIndex];
          existing.actualWeight = actualWeight;
          existing.actualReps = actualReps;
          existing.actualRpe = setDraft.actualRpe ? Math.min(10, Math.max(0, Number(setDraft.actualRpe))) || undefined : undefined;
          existing.setRating = rating;
          existing.notes = setDraft.notes || existing.notes;
    ...
    const loggedSet: LoggedSet = {
      ...
      added: nextPlannedSet?.notes?.toLowerCase().includes("added set") || effectiveSetIndex >= plannedSets.length,
  ...
  function previousSet() {
    const poppedSet = liveExerciseLog.sets[liveExerciseLog.sets.length - 1];
    void updateDb((draft) => {
      const target = draft.sessions.find((item) => item.id === liveSession.id);
      const log = target?.loggedExercises.find((item) => item.id === liveExerciseLog.id);
      log?.sets.pop();
    ...
    // Pre-fill the form with the popped set's actual values so data isn't silently lost
    if (poppedSet) {
      const newDraftKey = `${liveExerciseLog.id}:${Math.max(0, currentSetIndex - 1)}`;
      setSetDraft({
        draftKey: newDraftKey,
        kind: poppedSet.kind || "working" as SetKind,
        actualWeight: poppedSet.actualWeight ? String(poppedSet.actualWeight) : "",
        actualReps: poppedSet.actualReps ? String(poppedSet.actualReps) : "",
        actualRpe: poppedSet.actualRpe ? String(poppedSet.actualRpe) : "",
        setRating: poppedSet.setRating ?? 3 as SetRating,
        formRating: "4",
        muscleFeelRating: "4",
        pumpRating: "3",
        painRating: "0",
        sorenessRating: "2",
        notes: poppedSet.notes || ""
      });
    }
  }
        ...
        <div className="space-y-4">
          <section className="panel p-4">
                  ...
                  return (
                    <div
                      ...
                      onClick={() => {
                        ...
                        if (actual) {
                          // Load this set's actuals — for skipped sets, use planned values
                          setSetDraft((prev) => ({
                            ...prev,
                            draftKey: newDraftKey,
                            actualWeight: actual.skipped
                              ? (set.plannedWeight ? String(set.plannedWeight) : "")
                              : (actual.actualWeight ? String(actual.actualWeight) : ""),
                            actualReps: actual.skipped
                              ? (set.targetReps ? String(set.targetReps) : "")
                              : (actual.actualReps ? String(actual.actualReps) : ""),
                            actualRpe: actual.skipped
                              ? (set.targetRpe ? String(set.targetRpe) : "")
                              : (actual.actualRpe ? String(actual.actualRpe) : ""),
                            setRating: actual.skipped ? 3 as SetRating : (actual.setRating ?? 3 as SetRating),
                            notes: actual.skipped ? "" : (actual.notes || ""),
                          }));
                        } else {
                          // Pending set — load planned values
                          setSetDraft(emptySetDraft(set, liveExerciseLog.sets.at(-1), newDraftKey));
                        }
                      }}
            ...
            <div className="mt-4 grid grid-cols-2 gap-2 md:grid-cols-5">
              ...
              <button
                className="btn-secondary border-ember/40 text-orange-100"
                disabled={isPastLastPlannedSet || isEditingPastSet}
              ...
              >
                <Check className="h-5 w-5" /> {isEditingPastSet ? "Update Set" : isPastLastPlannedSet ? (hasMoreExercises ? "Next Exercise" : "Finish Workout") : primaryActionLabel}
              </button>
...
function WorkoutDayEditor({
  ...
  function chooseForMe() {
    ...
    const splitDay = allSplitDays.find((sd) => sd.id === day.splitDayId);
    const dayReqs = splitDay?.requirements ?? [];
...
function LibraryScreen({
  ...
  function saveEditExercise() {
    ...
    void updateDb((data) => {
      ...
      target.isCompound = draft.isCompound;
      target.kind = draft.isCompound ? ["compound"] : ["accessory"];
                    ...
                    <div key={exercise.id} className="rounded-lg border border-white/10 bg-white/[0.05] p-3">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <div className="flex shrink-0 gap-1">
                          {(exercise.ownerUserId === user.id || exercise.createdByUser) && (
...
function ExerciseProgressPanel({
  ...
  const loggedSetCount = completedSessions
    ...
    .flatMap((l) => l.sets.filter((s) => !s.skipped && s.kind !== "warmup"))
    .length;
  ...
  useEffect(() => {
    ...
    if (!hasAnyExercises && prevWeek) {
      ...
      void updateDb((draft) => {
        ...
        if (draftWeek && draftPrev) copyWeekExercises(draftPrev, draftWeek);
        return draft;
      });
      setCopied(true);
      ...
      {week.workouts.length > 1 && (
        <div className="flex flex-wrap gap-2">
          {week.workouts.map((day, idx) => {
            ...
            const tabLabel = `Day ${idx + 1}${daySplitDay?.name ? ` – ${daySplitDay.name}` : day.focus ? ` – ${day.focus}` : ""}`;
            return (
              <button
                key={day.id}
      ...
      {!activeProgram || !block || !week ? (
        ...
        <>
          ...
          {block.weeks.length > 1 && (
            <div className="flex flex-wrap gap-2">
              {block.weeks.map((w) => {
                const wSessions = db.sessions.filter((s) => s.userId === user.id && s.blockId === block.id && s.weekNumber === w.weekNumber);
                const wComplete = w.workouts.every((d) => wSessions.some((s) => s.workoutDayId === d.id && s.status === "completed") || block.skippedWorkoutDayIds?.includes(d.id) || d.status === "skipped");
                return (
                  <button
                    key={w.id}
                    className={`rounded-lg px-3 py-1.5 text-xs font-bold ${selectedWeekNumber === w.weekNumber ? "bg-volt text-iron-950" : wComplete ? "bg-white/10 text-volt" : "bg-white/[0.05] text-iron-400"}`}
                    onClick={() => setSelectedWeekNumber(w.weekNumber)}
                  >
                    Week {w.weekNumber}{w.weekNumber === currentWeekNumber ? " ●" : wComplete ? " ✓" : ""}
                  </button>
                );
              })}
            </div>
          ...
          {selectedWeekNumber === currentWeekNumber && isBlockWeekComplete(block, db.sessions.filter((s) => s.userId === user.id && s.blockId === block.id)) && (
            <WeekReviewPanel
              ...
              onPlanNextWeek={() => setPlanningWeekNumber(currentWeekNumber + 1)}
            />
          )}
          <Panel title={`Block Week ${week.weekNumber}${selectedWeekNumber !== currentWeekNumber ? " (completed)" : ""}`} icon={ClipboardList}>
            <div className="space-y-3">
...
function WeekReviewPanel({ block, sessions, onPlanNextWeek }: { block: TrainingBlock; sessions: WorkoutSession[]; onPlanNextWeek?: () => void }) {
...
function emptySetDraft(planned?: PlannedSet, last?: LoggedSet, draftKey = "") {
  // Carry forward the previous actual weight when the next planned set has the same
  // rep/RPE target (same effort) — this avoids reverting to the stale planned weight.
  const sameTarget =
    last &&
    planned &&
    (planned.targetRpe === undefined || last.targetRpe === undefined || planned.targetRpe === last.targetRpe) &&
    (planned.targetReps === undefined || last.plannedReps === undefined || planned.targetReps === last.plannedReps);
  const defaultWeight = sameTarget && last?.actualWeight
    ? String(last.actualWeight)
    : String(planned?.plannedWeight ?? last?.actualWeight ?? "");
  return {
    draftKey,
